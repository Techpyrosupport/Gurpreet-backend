...
const addBlog = async (req, res) => {
    try {
      ...
      let {title,subTitle,description,author,category} = req.body;
      ...
      if(categories){
        let foundCategory = categories.category.find(c=> c.toLowerCase()===category.toLowerCase());
        let data = [...categories.category]
        if(!foundCategory){
          data.push(category)
          await Category.findByIdAndUpdate(categories._id, { category:data})
        }
      }
      else{
        let data = [category];
        let categories = new Category({category:data});
...
const bulkInsertBlog = async (req,res)=>{
  try {
    ...
    for (let i = 0;i < dataToCreate.length;i++){
      ...
      if(dataToCreate[i].category){
        let foundCategory = data.find(c => c.toLowerCase()===dataToCreate[i].category.toLowerCase());
        if(!foundCategory){
          data.push(dataToCreate[i].category)
        }
      }
    }
    await Category.findByIdAndUpdate(categories._id, { category:data})
...
const updateBlog = async (req,res) => {
    try {
      ...
      if(dataToUpdate.category){
        const categories = await Category.findOne({});
        if(categories){
          let foundCategory = categories.category.find(c=> c.toLowerCase()===dataToUpdate.category.toLowerCase());
          let data = [...categories.category]
          if(!foundCategory){
            data.push(dataToUpdate.category)
            await Category.findByIdAndUpdate(categories._id, { category:data})
          }
        }
        else{
          let data = [dataToUpdate.category];
          let categories = new Category({category:data});
          await categories.save();
        }
      }
