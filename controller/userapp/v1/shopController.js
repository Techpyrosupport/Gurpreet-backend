...
const findNearestShop = async (req,res) => {
  try {
    let p = 1;
    let l = 10;
    const {latitude,longitude,radius,page,limit,sort} = req.body;
    if(!latitude || !longitude){
      return res.badRequest({ message : 'Insufficient request parameters! latitude and longitude is required.' });
    }
    if(page && Number(page)){
      p = page
    }
    ...
    let foundShops = await Shop.aggregate([
      {
        $geoNear: {
          near: {
            type: "Point",
          ...
          maxDistance: parseFloat(radius?radius:500)*1609,
          spherical: true
        }
      },
      {
    ...
    if (!foundShops || !foundShops.length || !(foundShops.length>0)){
      return res.recordNotFound(); 
    }
    return res.success({ data :foundShops[0] });
  } catch (error){
    return res.internalServerError({ message:error.message });
  }
};
