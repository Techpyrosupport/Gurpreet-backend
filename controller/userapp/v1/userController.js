...
const getAddressFromCoordinates = async (req, res) => {
  try {
    ...
    if (!lat || !lng) return res.failure({ message: "lat lng is required" });
    let data = await utils.getAddressFromLocation([lng, lat])
    return res.json(data)
  } catch (error) {
    console.log(error);
    res.failure({ message: "error from places API google" })
  }
}
